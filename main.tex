%--------------------------------------------------------%
% Journal Article Manuscript Template
%--------------------------------------------------------%

\input{sections/preamble} % Edit preamble.tex to change the overall layout

% Header from Page Three on: Edit below for left and right headers
% \lhead{}
% \rhead{}

%--------------------------------------------------------%
% BEGIN DOCUMENT
%--------------------------------------------------------%

\begin{document}

% COVER PAGE

\input{sections/coverpage} % Comment out to remove cover page

\thispagestyle{empty} % Removes header on page two.

% NOTE: Comment out the lines below to remove line numbers
  % Running line numbers:
  \linenumbers
  \setlength\linenumbersep{15pt}
  \renewcommand\linenumberfont{\normalfont\footnotesize\sffamily\color{gray}}
  %\pagewiselinenumbers % Same, but that reset on every page:
  \modulolinenumbers[1] % Number only every line. Change for fewer.

%--------------------------------------------------------%
%   CONTENT
%--------------------------------------------------------%

% ABSTRACT
\input{sections/abstract.tex}

\doublespacing

% INTRODUCTION
\input{sections/introduction.tex}

% MATERIALS AND METHODS
% REVISION: moved before Results per ISME Journal section order
\input{sections/methods.tex}

% RESULTS
\input{sections/results.tex}

% DISCUSSION
\input{sections/discussion.tex}

% ACKNOWLEDGMENTS
\input{sections/acknowledgements.tex}

% DATA AND CODE AVAILABILITY
\section*{Data Availability}

The data used in this study will be made publicly available upon publication.

\section*{Code Availability}

The machine learning pipeline and Python library are available at:
\url{https://github.com/kbasecollaborations/trait-prediction}.
The manuscript source, analysis scripts, and figure generation code are available at:
\url{https://github.com/dileep-kishore/trait-prediction-manuscript}.

%--------------------------------------------------------%
%   REFERENCE LIST
%--------------------------------------------------------%
\newpage
\singlespacing
\printbibliography

%--------------------------------------------------------%
%   APPENDIX
%--------------------------------------------------------%

%\input{sections/acronyms.tex}
\input{sections/tables_figures.tex}
\input{sections/supplementary.tex}

%--------------------------------------------------------%
%   END DOCUMENT
%--------------------------------------------------------%

\end{document}
