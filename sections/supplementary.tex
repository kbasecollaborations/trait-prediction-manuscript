...
The tree is drawn as a cladogram, so branch lengths are not to scale.
Phylogenetic distances are reported in Figure~\ref{fig:phylogenetic_distance_splits}.
The inner ring denotes dataset membership and the outer bar the GTDB order, coloured by GTDB class.
Orders with at least eight tips are labelled with their tip count, and tip labels are omitted for legibility.}
...
Tree distance (summed branch lengths) from each held-out test genome to its nearest training genome under random-holdout, in-clade, and out-of-clade splits, pooled across five splits per phenotype for the 15 shared phenotypes.
...
\caption{\textbf{Pangenome completeness was assessed for a subset of genomes, nearly all of which were near-complete.}
...
FB improved balanced accuracy over F (Figure~\ref{fig:bacdive_curation}A; 0.725 versus 0.677, paired Wilcoxon $p<0.001$, higher for 12 of 13 phenotypes) but was not significantly different from C (0.725 versus 0.706, $p=0.20$; MCC $p=0.53$; area under the ROC curve $p=0.99$), with higher precision offset by lower recall.
BacDive alone matched the concordant set (0.709 versus 0.706, $p=0.53$), whereas adding BacDive to the concordant set exceeded it (0.729 versus 0.706, $p=0.03$ across the 45 phenotype--held-out cells, $p=0.09$ across the 13 phenotype means).
...
C used fewer samples and produced more reproducible, mechanism-aligned feature rankings, a pattern consistent with reduced shortcut learning~\cite{Geirhos2020Shortcut}.
...
(\textbf{B}) Cross-seed feature stability, the mean pairwise Jaccard of the top-10 CatBoost-importance features across the five seeds; points are individual phenotype--held-out sets.
...
Pair-level logistic adjustment for dataset positive-call rates reduced but did not remove the association (adjusted odds ratio 2.69; 1.86 conditioning on both datasets' rates), and because comparisons reuse strains across pairs and phenotypes these odds ratios are descriptive, with independence-based confidence intervals and $p$-values not interpreted.
The analysis cannot distinguish assay differences, ecological or strain-history divergence, accessory-gene or regulatory differences, and curation effects, and does not show that concordance filtering specifically identifies the conflicting pairs.
