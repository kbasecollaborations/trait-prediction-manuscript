...
Strategies (leave-one-dataset-out; tested on each held-out manuscript dataset's
full labels):
    C   concordant samples of the other 3 manuscript datasets (curated)
    F   full other 3 manuscript datasets (no filter)
    FB  full other 3 + all BacDive (volume added)

Panels
------
A : Cross-dataset balanced accuracy. FB beats F significantly, but does NOT
    significantly beat the curated subset C.
B : Cross-seed feature stability (top-10 importance Jaccard). Adding volume
    makes feature selection significantly less reproducible (C > F > FB).
C : The trade-off, accuracy versus stability, one point per strategy.

The result tables (``data/outputs/bacdive/head_to_head.csv`` and
``strategy_stability.csv``) are produced by the BacDive analysis subsystem
deposited with the manuscript; this script only reads them and renders the
figure.

Run:
    uv run python -m scripts.figureS9.figureS9_plot
"""
...
def _panel_letter(ax: plt.Axes, letter: str) -> None:
    ...
    ax.text(-0.14, 1.08, letter, transform=ax.transAxes, fontweight="bold",
...
def _accuracy_cells() -> pd.DataFrame:
    ...
    df = df[(df["status"] == "ok") & (df["analysis"] == "strategy")
            & (df["strategy"].isin(ORDER))]
    return (df.groupby(["phenotype", "held_out", "strategy"])["balanced_accuracy"]
            .mean().unstack("strategy")[ORDER])
...
def _stability_cells() -> pd.DataFrame:
    ...
    top-feature sets across the five random seeds is computed.
    ...
    df["feats"] = df["top_features"].fillna("").apply(
    ...
    return cells.pivot_table(index=["phenotype", "held_out"],
                             columns="strategy", values="jac")[ORDER]
...
def _bracket(ax: plt.Axes, x1: float, x2: float, y: float, label: str,
             dy: float = 0.012) -> None:
...
def plot_figure(output_file: Path) -> None:
    ...
    # ---- Panel A: cross-dataset balanced accuracy ----
    ...
    _bracket(ax_a, 1, 2, top + 0.012, _stars(p_ffb_a))          # F vs FB
    _bracket(ax_a, 0, 2, top + 0.052, f"{_stars(p_cfb_a)} (p={p_cfb_a:.2f})")  # C vs FB
    ax_a.set_xticks(x)
    ...
    ax_b.bar(x, means_b, yerr=sems_b, color=colors, alpha=0.85, capsize=4,
             edgecolor="black", linewidth=0.6)
    for i, s in enumerate(ORDER):
        vals = stab[s].dropna().to_numpy()
        jitter = np.random.default_rng(0).uniform(-0.12, 0.12, size=len(vals))
        ax_b.scatter(np.full(len(vals), i) + jitter, vals, s=8,
                     color="black", alpha=0.25, zorder=3)
    ...
    # ---- Panel C: accuracy-vs-stability trade-off ----
    for s in ORDER:
    ...
    for s, (txt, dx, dy, ha, va) in point_labels.items():
        ax_c.text(stab[s].mean() + dx, acc[s].mean() + dy, txt,
                  fontsize=8, ha=ha, va=va, color=COLORS[s], fontweight="bold")
    ...
    print(f"  Panel A p-values: C-F={p_cf_a:.3f}  F-FB={p_ffb_a:.4f}  C-FB={p_cfb_a:.3f}")
    print(f"  Panel B p-value:  C-FB={p_cfb_b:.2e}")
    plt.close(fig)


if __name__ == "__main__":
    plot_figure(Path("figures/figure_s9.pdf"))
