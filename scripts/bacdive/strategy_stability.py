#!/usr/bin/env python3
"""Cross-dataset accuracy and top-K feature importances for the C / F / FB strategies.

Re-runs the head-to-head sweep fits and keeps the top-K feature importances per
fit so feature stability can be measured downstream.
...
def run_feature_job(job: dict[str, Any], thread_count: int = 1) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            **meta,
            **res,
            "top_features": ";".join(map(str, feats)),
            "status": "ok",
        }
...
def main() -> None:
    ...
    jobs, _ = build_jobs()
    jobs = [
        j for j in jobs if j["analysis"] == "strategy" and j["strategy"] in STRATEGIES
    ]
    print(
        f"Running {len(jobs)} fits (C/F/FB x 13 phenotypes x 4 held-out x 5 seeds), "
        f"n_jobs={N_JOBS}"
    )
    t0 = time.time()
    ...
    ok = (df["status"] == "ok").sum()
    print(
        f"saved {OUTPUT} ({ok} ok, {len(df) - ok} error) in {(time.time() - t0) / 60:.1f} min"
    )
    if ok:
        m = (
            df[df.status == "ok"]
            .groupby("strategy")["balanced_accuracy"]
            .agg(["mean", "count"])
            .round(3)
        )
        print(m.to_string())
