...
Kept in its own importable module (not the ``-m`` ``__main__`` driver) so that
``joblib``/``loky`` pickles ``run_job`` by reference rather than by value, and
workers resolve the cached loaders correctly.
"""
...
def run_job(job: dict[str, Any], thread_count: int) -> dict[str, Any]:
    ...
    try:
        ...
        result = perform_split_ml(
            ...
            # BacDive carbon sources are extremely positive-skewed; without this
            # the unweighted manuscript default collapses to majority-class
            # prediction (balanced accuracy ~0.5). See the design spec.
