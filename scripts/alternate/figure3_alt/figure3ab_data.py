#!/usr/bin/env python3
"""Generate intra-dataset (CV) and inter-dataset (train/test) comparison data for
Figure 3 from KOFAM features."""
    ...
    with tqdm(total=total_iterations, desc="Calculating CV results") as pbar:
        for feature in dataset.feature_set.features:
            for phenotype in dataset.phenotype_set.phenotypes:
                ...
                # Feature dataset must match phenotype dataset
            ...
            for test_key in keys:
                ...
                if test_size >= len(test_y):
                    updated_test_size = int(
                        len(test_y) * 0.8
                    )  # Fall back to 80% of available samples
                else:
...
def main() -> None:
    """Generate Figure 3 intra/inter-dataset data."""
