...
from scripts.figure7.applicability import (
    calibration_table,
...
def main() -> None:
    ...
    per_sample.to_csv(OUTPUT_DIR / "figure7_per_sample.tsv", sep="\t", index=False)
    risk_coverage.to_csv(
        OUTPUT_DIR / "figure7_risk_coverage.tsv", sep="\t", index=False
    )
    ...
    full_per_sample.to_csv(
        OUTPUT_DIR / "figure7_per_sample_fulldata.tsv", sep="\t", index=False
    )
    ...
    pd.concat(calib_rows, ignore_index=True).to_csv(
        OUTPUT_DIR / "figure7_calibration.tsv", sep="\t", index=False
    )
