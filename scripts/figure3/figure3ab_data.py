...
def calculate_cv_results(
    dataset: DataSet, model_type: str = "cb", n_splits: int = 5
) -> pd.DataFrame:
    """
    ...
    model_type : str, optional
        Type of model to use ('rf' or 'cb'), by default 'cb'
    ...
    for feature in tqdm(dataset.feature_set.features, desc="Processing features"):
        for phenotype in dataset.phenotype_set.phenotypes:
            ...
            cv_results = perform_cv(
                feature_data, phenotype_data, model_type, n_splits=n_splits
            )
...
def calculate_test_results(
    dataset: DataSet, model_type: str = "cb", test_size: int = 150, n_repeats: int = 5
) -> pd.DataFrame:
    """
    ...
    for train_key in tqdm(keys, desc="Processing train-test combinations"):
...
def main() -> None:
    ...
    if not cv_results_file.exists():
        cv_results = calculate_cv_results(dataset, model_type="cb", n_splits=5)
    ...
    if not test_results_file.exists():
        test_results = calculate_test_results(
            dataset, model_type="cb", test_size=150, n_repeats=5
        )
