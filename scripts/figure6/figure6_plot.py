...
RISK_FILE = DATA_DIR / "figure6_risk_coverage_by_phenotype.tsv"
CALIB_FILE = DATA_DIR / "figure6_calibration.tsv"
PRIOR_FILE = DATA_DIR / "figure6_prioritization.tsv"
...
def plot_risk_coverage(axes: list[Axes], risk: pd.DataFrame) -> None:
    ...
    risk : pd.DataFrame
        Contents of ``figure6_risk_coverage_by_phenotype.tsv``.
    """
...
def plot_calibration(ax: Axes, calib: pd.DataFrame) -> None:
    ...
    calib : pd.DataFrame
        Contents of ``figure6_calibration.tsv``.
    """
...
def plot_prioritization(ax: Axes, prior: pd.DataFrame) -> None:
    ...
    ax : Axes
        Matplotlib axes to draw on.
    prior : pd.DataFrame
        Contents of ``figure6_prioritization.tsv``.
    """
...
def plot_phenotype_priority(ax: Axes, prior: pd.DataFrame) -> None:
    ...
    ax : Axes
        Matplotlib axes to draw on.
    prior : pd.DataFrame
        Contents of ``figure6_prioritization.tsv`` (per-run rows).
    """
    budget = int(prior["n_added"].max())
    final = prior[prior["n_added"] == budget]
