...
Outputs (all under ``data/outputs/figure6/``):
    - ``figure6_per_sample.tsv``                  one row per held-out test genome
    - ``figure6_risk_coverage.tsv``               pooled balanced accuracy vs. coverage
    - ``figure6_risk_coverage_by_phenotype.tsv``  per-phenotype risk-coverage curves
"""
...
from scripts.figure6.applicability import (
    calibration_table,
...
def main() -> None:
    ...
    per_sample.to_csv(OUTPUT_DIR / "figure6_per_sample.tsv", sep="\t", index=False)
    risk_coverage.to_csv(
        OUTPUT_DIR / "figure6_risk_coverage.tsv", sep="\t", index=False
    )
    ...
    full_per_sample.to_csv(
        OUTPUT_DIR / "figure6_per_sample_fulldata.tsv", sep="\t", index=False
    )
    ...
    pd.concat(calib_rows, ignore_index=True).to_csv(
        OUTPUT_DIR / "figure6_calibration.tsv", sep="\t", index=False
    )
    ...
    pd.concat(rc_rows, ignore_index=True).to_csv(
        OUTPUT_DIR / "figure6_risk_coverage_by_phenotype.tsv", sep="\t", index=False
    )
