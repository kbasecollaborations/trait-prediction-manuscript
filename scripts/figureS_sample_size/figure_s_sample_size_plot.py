...
# Change this line to match the feature type used in figure_s_sample_size_data.py
FEATURE_TYPE = "kofam"

# Sample sizes used in figure_s_sample_size_data.py
SAMPLE_SIZES = [50, 100, 200, 500, "full"]

# Manuscript figure routing: only the Histidine combined-test grid appears in
# the manuscript (as figure_s_sample_size.pdf). All other variants — the Full/Concordant/
...
MANUSCRIPT_FIGURE_NAME = "figure_s_sample_size.pdf"
...
def plot_all_test_subsets(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for test_subset in test_subsets:
        ...
        output_file = output_dir / f"figure_s_sample_size_{subset_name}.pdf"
...
def plot_combined_test_subsets(
    ...
    for phenotype in phenotypes:
        ...
        if phenotype == MANUSCRIPT_PHENOTYPE:
            ...
            phenotype_file = (
                alternate_dir / f"figure_s_sample_size_{phenotype.lower()}_all_tests.pdf"
            )
...
def main() -> None:
    ...
    # Load data
    data_file = Path(
        f"data/outputs/figureS_sample_size/figure_s_sample_size_data_requirements_{FEATURE_TYPE}.csv"
    )
    ...
    # Create output directories. The manuscript figure (figure_s_sample_size.pdf) lives in
    ...
    plot_performance_vs_sample_size(
        plot_data, alternate_dir / "figure_s_sample_size_full_test_2x3.pdf", test_subset="Full Test"
    )
    ...
    # figures/figure_s_sample_size.pdf; the rest land in figures/alternate/.
