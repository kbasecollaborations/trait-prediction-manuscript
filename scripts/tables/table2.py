...
def _format_clustered_kos(
    ...
    str
        ...
        cluster sibling (singletons) are listed last.
    """
    if not kos:
        return "None"
...
def create_feature_table(
    ...
    """Write the LaTeX feature-comparison table for the concordant-data analysis.
    ...
    latex_lines.append(
        ...
        "for selected phenotypes when trained on GapMind-concordant samples "
        "only, after screening each model to a broad candidate set by CatBoost "
        "feature importance. \\textbf{Shared stable features} are KOs present "
        "in both the concordant-trained model fit on the three non-held-out "
        "datasets and the concordant-trained model fit on the held-out dataset "
        "alone. \\textbf{Unique to held-out-alone model} are KOs present only "
        "when training on the held-out dataset alone. KOs are bracketed with a "
